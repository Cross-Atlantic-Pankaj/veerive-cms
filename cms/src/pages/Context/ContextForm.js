                                    ...
                                    <div style={{ flex: 1 }}>
                                        ...
                                        <input
                                            ...
                                            onChange={async (e) => {
                                                ...
                                                try {
                                                    const res = await axios.post('/api/images/upload-ppt', formData, {
                                                        headers: { 
                                                            'Content-Type': 'multipart/form-data',
                                                            Authorization: `Bearer ${sessionStorage.getItem('token') || localStorage.getItem('token')}`
                                                        }
                                                    });
                                                    if (res.data.success && res.data.data?.pptUrl) {
                                                        setPptUrl(res.data.data.pptUrl);
                                                        toast.success('PPT uploaded successfully!');
                                                    } else {
                                                        toast.error(res.data.message || 'Failed to upload PPT');
                                                    }
                                                } catch (err) {
